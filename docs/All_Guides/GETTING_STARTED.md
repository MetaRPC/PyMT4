# Getting Started with PyMT4

> **Quick Setup & Overview** - Start automating your trading with PyMT4 (Python for MetaTrader 4) in minutes.

---

## 🎯 What is PyMT4?

**PyMT4** is an industrial-grade, type-safe Python client library for interacting with **MetaTrader 4** terminals via high-performance **gRPC**. It eliminates complex C++ DLL wrappers and provides direct, reliable programmatic trading.
...
Install PyMT4 via your standard Python package manager:

```bash
pip install metarpc-mt4
```
...
```
account = MT4Account(user=user, password=password, host=grpc_server)
...
To get the most out of PyMT4, follow this suggested reading order:
